...
import { createFromCapture } from "@/services/api/measurement-profiles";
import {
...
export default function CreateMeasurementScreen() {
  ...
  const { addProfileFromServer } = useMeasurementProfiles();
  ...
  const canProceedStep1 = name.trim() && gender && relationship;
  ...
  const canSave =
    frontImageUri != null &&
    frontImageUri.length > 0 &&
    sideImageUri != null &&
    sideImageUri.length > 0;
  ...
  const handleNext = useCallback(async () => {
    ...
    if (!frontImageUri || !sideImageUri) {
      Alert.alert(
        "Photos required",
        "Please add both front view and side view photos.",
      );
      return;
    }
    setSaveError(null);
    setSaving(true);
    try {
      const res = await createFromCapture({
        name: name.trim(),
        gender: gender as GenderValue,
        frontViewUri: frontImageUri,
        sideViewUri: sideImageUri,
      ...
      addProfileFromServer({
        id: data?.id != null ? String(data.id) : `mp_${Date.now()}`,
        name: name.trim(),
        gender: gender as GenderValue,
        relationship,
        relationshipCustom:
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <PageHeader onBack={handleBack} title="New profile" variant="close" />
      <ScrollView
        style={styles.scroll}
      ...
      >
        ...
        <View style={styles.progressWrap}>
          <View
            style={[styles.progressBar, { width: `${(step / STEPS) * 100}%` }]}
          />
        </View>
        ...
        {step === 1 && (
          <>
            ...
            <Text style={styles.screenSubtitle}>
              Name and relationship help us organize your measurements.
            </Text>
            <Input
              label="Profile name"
        ...
        {step === 2 && (
          <>
            ...
            <Text style={styles.screenSubtitle}>
              Front and side views help us tailor your fit. Use a plain
            ...
            <Text style={styles.screenSubtitle}>
              Optional. Improves size recommendations.
            </Text>
            <Input
              label="Height (cm)"
              placeholder="e.g. 175"
        ...
        {step === 3 && (
          <>
            ...
            <Text style={styles.screenSubtitle}>
              Save this profile to your list. You can edit it anytime.
            </Text>
            {saveError ? (
              <View style={styles.errorBanner}>
                <Text style={styles.errorBannerText}>{saveError}</Text>
                <Text style={styles.errorBannerHint}>
                  Tap Save profile below to retry.
                </Text>
              </View>
            ) : null}
            <View style={styles.summaryCard}>
              ...
              <Text style={styles.summaryMeta}>
                {gender === "male" ? "Male" : "Female"} · {relationshipLabel}
              </Text>
              {(heightNum != null || weightNum != null) && (
                <Text style={styles.summaryMeta}>
                  {heightNum != null ? `${heightNum} cm` : ""}
                  {heightNum != null && weightNum != null ? " · " : ""}
                  {weightNum != null ? `${weightNum} kg` : ""}
                </Text>
              )}
            </View>
          </>
        ...
        <View style={styles.footer}>
          <Button
            title={
              step < STEPS ? "Continue" : saving ? "Saving…" : "Save profile"
            }
            onPress={handleNext}
            variant="primary"
            fullWidth
            disabled={(step === 1 && !canProceedStep1) || saving}
